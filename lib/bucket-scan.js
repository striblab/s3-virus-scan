...
module.exports = function(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Ability to scan one key (does not affect collection)
    if (options.key) {
      if (options.spinner) {
        options.spinner.text = `Scanning one... | ${options.key}`;
      }
    ...
    keyStream.on('data', key => {
      ...
      clamavService.scan(s3Stream, (error, object, malicious) => {
        // If S3 throws error, but we still end up here, don't count
        if (key.error) {
          return;
        }
...
// Scan one.  Obviously, the code shoud be better where this
